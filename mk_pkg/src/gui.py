...
class Main(QMainWindow):
    def __init__(self):
        ...
        self.switch_topic   = rospy.get_param("~switch_topic", "/flag/switch")
        ...
        self.pick_topic   = rospy.get_param("~pick_topic", "/flag/pick_flag")
        self.place_topic   = rospy.get_param("~place_topic", "/flag/place_flag")
        ...
        self.pick_flag_pub   = rospy.Publisher(self.pick_topic, Bool, queue_size=10)
        self.place_flag_pub   = rospy.Publisher(self.place_topic, Bool, queue_size=10)
        ...
        # 버튼 총괄 그룹
        button_group_all_h = QWidget()
        hbtn_all= QHBoxLayout(button_group_all_h)
        
        #버튼 1열
        button_group_cal_1 = QWidget()
        vbtn_cal_1 = QVBoxLayout(button_group_cal_1)

        self.btn_pick  = QPushButton("Pick")
        self.btn_place  = QPushButton("Place")

        vbtn_cal_1.addWidget(self.btn_pick)
        vbtn_cal_1.addWidget(self.btn_place)
        vbtn_cal_1.setStretch(0, 1)
        vbtn_cal_1.setStretch(1, 1)
        
        vbtn_cal_1.setSpacing(20)
        for layout in (vbtn_cal_1, ):
            for i in range(layout.count()):
                btn = layout.itemAt(i).widget()
                btn.setSizePolicy(QSizePolicy.Expanding, QSizePolicy.Expanding)
        
        hbtn_all.addWidget(button_group_cal_1, 1)
        ...
        hbtn_all.addWidget(vbtn_group, 7)
        
        
        button_group_all_v = QWidget()
        vbtn_all = QVBoxLayout(button_group_all_v)
        self.color_box = QFrame()
        self.color_box.setSizePolicy(QSizePolicy.Expanding, QSizePolicy.Expanding)
        self.set_state_color("red")
        vbtn_all.addWidget(button_group_all_h, 4)
        vbtn_all.addWidget(self.color_box, 1)


        vright.addWidget(button_group_all_v, 1)
        ...
        self.btn_pick.clicked.connect(lambda: self.publish_pick(True))
        self.btn_place.clicked.connect(lambda: self.publish_place(True))
        ...
        self.btn_pick.clicked.connect(lambda: self.set_state_color("red"))
        self.btn_place.clicked.connect(lambda: self.set_state_color("red"))
    ...
    def publish_pick(self, state: bool):
        try:
            self.switch_pub.publish(Bool(data=False))
            self.place_flag_pub.publish(Bool(data=False))
            self.pick_flag_pub.publish(Bool(data=state))
        except Exception as e:
            rospy.logerr(f"pick_flag_pub error: {e}")
            
    def publish_place(self, state: bool):
        try:
            self.switch_pub.publish(Bool(data=False))
            self.pick_flag_pub.publish(Bool(data=False))
            self.place_flag_pub.publish(Bool(data=state))
        except Exception as e:
            rospy.logerr(f"place_flag_pub error: {e}")
